...
# -*- coding: utf-8 -*-
"""
智能文档问答助手 - 基于LingyeAgents的智能文档问答系统
...
from lingye_agent.tools import RAGTool, MemoryTool
import gradio as gr
...
def create_gradio_ui():
    ...
    def chat(message: str, history: List) -> Tuple[str, List]:
        """聊天功能（Gradio 6+ 使用 role/content 消息列表，不再使用 [user, bot] 元组）"""
        hist = list(history) if history else []

        if assistant_state["assistant"] is None:
            hist.append({"role": "user", "content": message})
            hist.append({"role": "assistant", "content": "❌ 请先初始化助手并加载文档"})
            return "", hist
...
def _ensure_localhost_proxy_bypass() -> None:
    """系统开启 HTTP/HTTPS 代理时，httpx 可能把对 localhost 的请求也走代理，导致 Gradio 自检 502。"""
    extra = ("localhost", "127.0.0.1", "::1", "0.0.0.0")
...
def main():
    ...
    print("="*60)
    print("正在启动Web界面...\n")
    print(f"本机地址: http://{server_name}:{server_port}\n")
