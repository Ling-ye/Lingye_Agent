...
class ContextBuilder:
    ...
    def _select(
        ...
        """Select: 基于分数与预算的筛选"""
        # 1) 计算相关性（jieba 分词 + 词集合重叠，适配中文）
        query_tokens = {t.strip() for t in jieba.lcut(user_query.lower()) if t.strip()}
        for packet in packets:
            content_tokens = {t.strip() for t in jieba.lcut(packet.content.lower()) if t.strip()}
