...
- **核心层 (`lingye_agent/core/`)**：LLM 抽象、Agent 基类、流式事件、生命周期钩子。
- **Agent 层 (`lingye_agent/agents/`)**：内置 6 种主流 Agent 范式（Simple / FunctionCall / ReAct / Plan-Solve / Reflection / ContextAware）。
- **工具层 (`lingye_agent/tools/`)**：可插拔工具体系 + `@tool_action` 自动展开 + MCP 协议接入。
- **记忆层 (`lingye_agent/memory/`)**：4 类记忆（工作 / 情景 / 语义 / 感知）+ Qdrant 向量库 + Neo4j 图库 + 多源 embedding。
- **上下文层 (`lingye_agent/context/`)**：GSSC（Gather-Select-Structure-Compress）流水线，按 token 预算智能裁剪上下文。
- **协议层 (`lingye_agent/protocols/`)**：MCP 客户端，支持 stdio / HTTP / SSE / 内存四种传输。
- **示例层 (`examples/`)**：5 个端到端示例（PDF 学习助手、代码库维护、文档生成、Tavily 搜索…）。
...
```bash
pip install lingye-agent            # 核心包（Agent + LLM + 基础工具）
pip install lingye-agent[rag]       # + RAG（Qdrant + embedding）
pip install lingye-agent[mcp]       # + MCP 协议
pip install lingye-agent[all]       # 安装全部可选依赖
```
...
```python
from lingye_agent import LingyeLLM, FunctionCallAgent, ToolRegistry
...
from lingye_agent.agents import ContextAwareAgent  # 需要 pip install lingye-agent[rag,nlp]
...
### `lingye_agent/agents/` — Agent 范式实现

每个 Agent 都继承 `lingye_agent.core.Agent`，支持：
...
### `lingye_agent/tools/` — 工具体系

```python
from lingye_agent import Tool, ToolRegistry, tool_action
...
### `lingye_agent/context/` — GSSC 上下文工程
...
### `lingye_agent/protocols/` — MCP 客户端

```python
from lingye_agent.protocols.mcp import MCPClient
...
```python
from lingye_agent import LingyeLLM, ReActAgent, ToolRegistry
from lingye_agent.tools import simple_calculate
...
```python
from lingye_agent import PlanAndSolveAgent, LingyeLLM
...
from lingye_agent import SimpleAgent, LingyeLLM
...
```python
import asyncio
from lingye_agent import ReflectionAgent, LingyeLLM
...
| 应用 | 说明 | 入口 |
|---|---|---|
| **PDF 学习助手** | Gradio Web UI，加载 PDF + RAG 问答 + 学习记忆回顾 | `examples/PDF_learning_assistant.py` |
| **代码库维护助手** | FunctionCall + Terminal + Note + Memory 长程探索代码库 | `examples/codebase_maintainer.py` |
| **项目助手** | NoteTool + ContextBuilder 的长期项目管家 | `examples/project_assistant.py` |
| **多 Agent 文档生成** | GitHub 搜索 Agent + 文档撰写 Agent 协作 | `examples/doc_assistant.py` |
| **Tavily 搜索助手** | MCP 接入 Tavily 的极简示例 | `examples/tavily_search.py` |

运行示例：

```bash
python examples/PDF_learning_assistant.py
python examples/codebase_maintainer.py
```
...
```python
from lingye_agent import Tool, ToolParameter
...
```python
from lingye_agent import Agent, Message
...
继承 `lingye_agent.memory.BaseMemory`，实现 `add / retrieve / update / remove / has_memory / clear / get_stats` 七个抽象方法即可。
...
```
Lingye_Agent/
├── lingye_agent/                # 可安装的 Python 包（pip install lingye-agent）
│   ├── __init__.py              # 聚合导出 + __version__
│   ├── core/                    # 框架基础
│   │   ├── agent.py             # Agent 抽象基类
...
│           └── server.py        # MCP 服务端
│
├── examples/                    # 示例应用（不随 pip install 分发）
...
├── pyproject.toml               # 包构建配置（pip install 入口）
├── requirements.txt             # 旧式依赖清单（保留兼容）
├── .env.example                 # 环境变量模板
├── LICENSE
└── README.md
```
...
- [x] GSSC 上下文工程
- [x] **包结构规范化**：`pip install lingye-agent` + 可选依赖分组 + `pyproject.toml`
